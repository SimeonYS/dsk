...
class DskPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('dsk.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dsk`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dsk WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dsk (date, title, link, content)"
